...
<body>
    ...
    <script>
        ...
        function limpiarFormulario() {
            // Limpia completamente el formulario y vuelve a 1 sección vacía
        ...
        // --- Recalcular en servidor si ya había resultado mostrado ---
        function hayResultadoMostrado() {
            return !!document.getElementById('bloque-resultado');
        }

        function recomputarSiCorresponde() {
            // Recalcula solo si ya se había mostrado un resultado (después de 1er submit)
        ...
        // --- Auto-cálculo al completar secciones (con debounce) ---
        ...
        function seccionEsValida(card) {
            const m2 = card.querySelector('input[name*="[m2]"]');
            const estado = card.querySelector('select[name*="[estado_id]"]');
            const m2val = m2 && m2.value !== '' && !isNaN(m2.value) && parseFloat(m2.value) > 0;
            const estval = estado && estado.value !== '';
            return m2val && estval;
        }

        function enlazarAutoCalculo(card) {
            // Cuando el usuario rellene una sección y sea válida, recalculamos automáticamente
            const inputs = card.querySelectorAll('input[name*="[m2]"], select[name*="[estado_id]"]');
            inputs.forEach(el => {
                el.addEventListener('change', () => {
                    if (hayResultadoMostrado() && seccionEsValida(card)) autoSubmitDebounced();
                    renumerarTitulos(); // mantener encabezado sincronizado con el título escrito
                });
                el.addEventListener('blur', () => {
                    if (hayResultadoMostrado() && seccionEsValida(card)) autoSubmitDebounced();
                });
                el.addEventListener('input', () => {
                    // Si ya hay resultado y el user está tipeando m2, recalculamos apenas sea válido
                    if (hayResultadoMostrado() && seccionEsValida(card)) autoSubmitDebounced();
                });
            });
        }
        ...
        function eliminarSeccion(btn) {
            ...
            if (container.children.length === 0) {
                agregarSeccion(); // mantener siempre al menos 1
            }
            renumerarTitulos();
        ...
        `;
            ...
            // IMPORTANTE:
            // No recalculamos inmediatamente al agregar, porque la sección está vacía
            // y gatillaría validaciones. Apenas el usuario complete m² y estado, se recalcula solo.
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            // Enlazar auto-cálculo a todas las secciones ya renderizadas por el servidor
